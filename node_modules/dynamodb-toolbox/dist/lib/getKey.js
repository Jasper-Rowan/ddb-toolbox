...
exports.default = () => (data, schema, partitionKey, sortKey) => {
    ...
    if (pk === undefined || pk === null || pk === '') {
        (0, utils_1.error)(`'${partitionKey}'${schema[partitionKey].alias ? ` or '${schema[partitionKey].alias}'` : ''} is required`);
    }
    const sk = data[sortKey];
    if (sortKey && (sk === undefined || sk === null || sk === '')) {
        (0, utils_1.error)(`'${sortKey}'${schema[sortKey].alias ? ` or '${schema[sortKey].alias}'` : ''} is required`);
    }
    return Object.assign({
        [partitionKey]: (0, utils_1.transformAttr)(schema[partitionKey], validateType(schema[partitionKey], partitionKey, pk), data)
    }, sortKey !== null
        ? {
            [sortKey]: (0, utils_1.transformAttr)(schema[sortKey], validateType(schema[sortKey], sortKey, sk), data)
        }
        : {});
};
