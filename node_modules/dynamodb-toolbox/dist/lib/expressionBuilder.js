...
exports.SUPPORTED_FILTER_EXP_ATTR_REF_OPERATORS = void 0;
const checkAttribute_1 = __importDefault(require("./checkAttribute"));
const utils_1 = require("./utils");
exports.SUPPORTED_FILTER_EXP_ATTR_REF_OPERATORS = ['eq', 'ne', 'lt', 'lte', 'gt', 'gte'];
const toDynamoValue = (value) => {
    if (typeof value === 'bigint') {
        return (0, utils_1.toDynamoBigInt)(value);
    }
    return value;
};
...
const conditionError = (op) => (0, utils_1.error)(`You can only supply one filter condition per query. Already using '${op}'`);
...
const parseClause = (_clause, grp, table) => {
    ...
    if (Object.keys(args).length > 0)
        (0, utils_1.error)(`Invalid expression options: ${Object.keys(args).join(', ')}`);
    // Verify entity
    if (entity !== undefined &&
        ...
        (0, utils_1.error)(`'entity' value of '${entity}' must be a string and a valid table Entity name`);
    const path = typeof attr === 'string'
        ? (0, checkAttribute_1.default)(attr, entity ? table[entity].schema.attributes : table.Table.attributes)
        : typeof size === 'string'
            ? (0, checkAttribute_1.default)(size, entity ? table[entity].schema.attributes : table.Table.attributes)
            : (0, utils_1.error)(`A string for 'attr' or 'size' is required for condition expressions`);
    ...
    if (operator && filterType) {
        ...
        if (operator === 'BETWEEN') {
            ...
            if (Array.isArray(value) && value.length === 2) {
                ...
                (0, utils_1.error)(`'between' conditions require an array with two values.`);
            }
        }
        else if (operator === 'IN') {
            if (!attr)
                (0, utils_1.error)(`'in' conditions require an 'attr'.`);
            // Verify array input
            if (Array.isArray(value)) {
                ...
                (0, utils_1.error)(`'in' conditions require an array.`);
            }
        }
        else if (operator === 'EXISTS') {
            if (!attr)
                (0, utils_1.error)(`'exists' conditions require an 'attr'.`);
            clause = value ? `attribute_exists(${operand})` : `attribute_not_exists(${operand})`;
        }
        else if (value && typeof value === 'object') {
            const ref = value;
            if (!exports.SUPPORTED_FILTER_EXP_ATTR_REF_OPERATORS.includes(filterType))
                (0, utils_1.error)(`AttrRef is only supported for the following operators: ${exports.SUPPORTED_FILTER_EXP_ATTR_REF_OPERATORS.join(', ')}.`);
            if (typeof (ref === null || ref === void 0 ? void 0 : ref.attr) !== 'string' || !(ref === null || ref === void 0 ? void 0 : ref.attr))
                (0, utils_1.error)(`AttrRef must have an attr field which references another attribute in the same entity.`);
            names[`#attr${grp}_ref`] = (0, checkAttribute_1.default)(ref.attr, entity ? table[entity].schema.attributes : table.Table.attributes);
            ...
            if (operator === 'BEGINS_WITH') {
                if (!attr)
                    (0, utils_1.error)(`'beginsWith' conditions require an 'attr'.`);
                clause = `begins_with(${operand},:attr${grp})`;
            }
            else if (operator === 'CONTAINS') {
                if (!attr)
                    (0, utils_1.error)(`'contains' conditions require an 'attr'.`);
                clause = `contains(${operand},:attr${grp})`;
            }
            else if (operator === 'ATTRIBUTE_TYPE') {
                if (!attr)
                    (0, utils_1.error)(`'type' conditions require an 'attr'.`);
        ...
        (0, utils_1.error)('A condition is required');
    }
    return {
        logic: or ? 'OR' : 'AND',
        clause,
